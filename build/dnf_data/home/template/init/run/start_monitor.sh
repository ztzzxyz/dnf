...
# 导致stdout块缓冲永不刷新, monitor插件的输出[on_load/等待触发等]全部看不到).
# stdbuf -oL -eL 将stdout/stderr设为行缓冲, 使日志实时写入monitor.log
# LD_PRELOAD放在stdbuf之前(stdbuf会保留并前置libstdbuf.so)
# libfd_monitor.so为monitor插件(参考70s3a3分支), 挂载在/home/neople/monitor目录下
