...
--------------------------------------------------------------------------强化增幅必成----------------------------------------------------------------------------
--强化/增幅失败时，减少一个材料【2660374】并进行一次判定，通过则强制改为强化/增幅成功，未通过或无材料则仍然失败
local function MyUpgrade(fnext, _user, iitem)
    ...
    if not ok then
		local inven = user:GetCurCharacInvenR()
	    if inven:CheckItemExist(2660374) < 1 then
		else
		
			local list1 = {2660374} -- 可以回收的id
			...
			if q < 1 then --若随机的值小于5，强化/增幅任然失败
				...
				local logFile = io.open("/dp2/script/rizhi.log", "a")  
         	    ...
         	    iitem:inc_upgrade()
         	    ok = true
		 	    sendPacketMessage(string.format("—————————精灵的祝福—————————\n 玩家【%s】 在氪金之力祝福下 %s成功啦!", user:GetCharacName(), iitem:has_amplify_ability() and "增幅" or "强化"), 15)
		 	    local logFile = io.open("/dp2/script/rizhi.log", "a")  
   		 	        if logFile then
...
item_handler[10000209] = function(user, item_id)
    local quest = dpx.quest
    local lst = quest.all(user.cptr)
    local chr_level = user:GetCharacLevel()
    local new_lst = {}
...
item_handler[490703503] = function(user, item_id)
    local quest = dpx.quest
    local lst = quest.all(user.cptr)
    local chr_level = user:GetCharacLevel()
    local new_lst = {}
...
--以下为装备回收，不用就删掉这一段--
item_handler[26603755] = function(user, item_id)
	local list1 = {28316, 27347, 29155, 30043, 31150, 31440, 32034, 32345, 34009, 34010, 33414, 35109, 36305, 35401, 27749, 29747, 33109, 37392} -- 可以回收的装备id
    local list2 = {3336} -- 回收奖励id
        ...
        if info then
            for _, _equip in ipairs(list1) do
                if info.id == _equip then
                    ...
                    local count = math.random(100, 100)
...
--以下为装备回收，不用就删掉这一段--
item_handler[2660364] = function(user, item_id)
	local list1 = {27149, 28042, 29396, 30310, 32346, 32041, 32040, 31446, 31156, 33709, 33710, 34017, 35703, 37105, 27750, 34290, 37694, 31738, 35997} -- 可以回收的装备id
    local list2 = {3337} -- 回收奖励id
    local to_recycle = {} -- 待回收装备列表
	local q = 0
    math.randomseed(tostring(os.time()):reverse():sub(1, 7))
...
--以下为装备回收，不用就删掉这一段--
item_handler[2660365] = function(user, item_id)
	local list1 = {101030033, 101040034, 101000018, 101020038, 101010036, 102000044, 102030036, 102000045, 102010103, 102020037, 102040037, 104020033, 104000037, 104030039, 104040036, 104020034, 106020032, 106000034, 106000035, 106030037, 108000034, 108020032, 108040033, 108030033, 109010032, 109000034, 109020032, 109020033} -- 可以回收的装备id
    local list2 = {3338} -- 回收奖励id
    local to_recycle = {} -- 待回收装备列表
	local q = 0
    math.randomseed(tostring(os.time()):reverse():sub(1, 7))
...
--以下为装备回收，不用就删掉这一段--
item_handler[2660366] = function(user, item_id)
	local list1 = {101020039, 101030032, 101000019, 101040035, 104000038, 104010037, 104040041, 102020039, 102030035, 106040033, 106010040, 106020038, 108000035, 108010039, 108020033, 109010033} -- 可以回收的装备id
    local list2 = {3339} -- 回收奖励id
    local to_recycle = {} -- 待回收装备列表
	local q = 0
    math.randomseed(tostring(os.time()):reverse():sub(1, 7))
...
--以下为装备回收，不用就删掉这一段--
item_handler[2660367] = function(user, item_id)
	local list1 = {100050051, 100100013, 100070012, 100120011, 100180011, 100230012, 100050052, 100100014, 100060018, 100110013, 100060019, 100070013, 100120012, 100080011, 100130011, 100090011, 100140011, 100300063, 100300064, 100300065, 100300067, 100300066, 100310061, 100320064, 100320067, 100320065, 100320068, 100350095, 100350094, 100350096, 100350097, 100340324, 100340325, 100340326} -- 可以回收的装备id
    local list2 = {3338} -- 回收奖励id
    local to_recycle = {} -- 待回收装备列表
	local q = 0
    math.randomseed(tostring(os.time()):reverse():sub(1, 7))
...
--以下为装备回收，不用就删掉这一段--
item_handler[2660384] = function(user, item_id)
	local list1 = {100050070, 100060026, 100070020, 100080016, 100090017, 100300076, 100300078, 100310070, 100310071, 100320077, 100320078, 100340348, 100350104, 100350105, 100350106} -- 可以回收的装备id
    local list2 = {3339} -- 回收奖励id
    local to_recycle = {} -- 待回收装备列表
	local q = 0
    math.randomseed(tostring(os.time()):reverse():sub(1, 7))
...
--以下为装备回收，不用就删掉这一段--
item_handler[2660385] = function(user, item_id)
	local list1 = {10974, 12968, 11355, 13353, 10572, 12569, 19705, 22233, 24275, 440366, 440364, 440365, 12197, 10205, 12568, 10571, 450120, 450119, 450118} -- 可以回收的装备id
    local list2 = {3337} -- 回收奖励id
    local to_recycle = {} -- 待回收装备列表
	local q = 0
    math.randomseed(tostring(os.time()):reverse():sub(1, 7))
...
--以下为装备回收，不用就删掉这一段--
item_handler[2660386] = function(user, item_id)
	local list1 = {10203, 10204, 12195, 12196, 10570, 12567, 11354, 13352, 11708, 13707, 11709, 13708, 20247, 440351, 10973, 12967} -- 可以回收的装备id
    local list2 = {3336} -- 回收奖励id
    local to_recycle = {} -- 待回收装备列表
	local q = 0
    math.randomseed(tostring(os.time()):reverse():sub(1, 7))
...
item_handler[10000217] = function(user, item_id)
    local cera_count = 1000
    user:ChargeCera(cera_count)
    user:SendNotiPacketMessage(string.format("已充值%d点券！", cera_count), 14)
    local logFile = io.open("/dp2/script/rizhi.log", "a")  
...
--以下为一次觉醒券，不用就删掉这一段--
item_handler[2660375] = function(user, item_id)
	local level = user:GetCharacLevel()
    local growType = user:GetCharacGrowType()
        if growType > 6 then
		    dpx.item.add(user.cptr, item_id)
...
-- 以下是70级直升券代码 升到70级并清理前69级任务，可在70级前任意一级使用（不用就删掉这一大段）!
item_handler[490703501] = function(user, item_id)
    local currentLevel = user:GetCharacLevel()
    if currentLevel >= 70 then
        dpx.item.add(user.cptr, item_id)
        user:SendNotiPacketMessage("\n当前用户已满70级，无法使用70级直升券！", 1)
    else
        for i = currentLevel, 68, 1 do
            user:AddCharacExpPercent(1)
        end
        logger.info(string.format("%d 升到69级", user:GetCharacNo()))
        local quest = dpx.quest
        local lst = quest.all(user.cptr)
        local chr_level = user:GetCharacLevel()
        --下面{}内写要排除的任务编号即可，就不会自动完成如下编号的任务，例子是辅助装备魔法石
        local evade_lst = { 6931, 649 }
        for i, v in ipairs(lst) do
            for j, w in ipairs(evade_lst) do
                if v == w then
                    table.remove(lst, i)
        ...
        quest.update(user.cptr)
        logger.info(string.format("%d 69级主线清理完成", user:GetCharacNo()))
        user:AddCharacExpPercent(1)
        logger.info(string.format("%d 升到70级", user:GetCharacNo()))
    end
end
-- 以上是70级直升券代码 升到70级并清理前69级任务（不用就删掉这一大段）!

-- 以下是70级胶囊
item_handler[690000039] = function(user, item_id)
    local currentLevel = user:GetCharacLevel()
    if currentLevel >= 70 then
        dpx.item.add(user.cptr, item_id)
        user:SendNotiPacketMessage("\n当前用户已满70级，无法使用70级直升！", 1)
    else
        for i = currentLevel, 68, 1 do
            user:AddCharacExpPercent(1)
        end
...
-- 以下是70级直升券代码 升到20级并清理前19级任务，可在70级前任意一级使用（不用就删掉这一大段）!
item_handler[690000096] = function(user, item_id)
    local currentLevel = user:GetCharacLevel()
...
item_handler[20220513] = function(user, item_id)
    user:ResetDimensionInout(0)
    user:ResetDimensionInout(1)
    user:ResetDimensionInout(2)
	user:SendNotiPacketMessage("恭喜： 异界E2重置 成功！")
end
item_handler[20220512] = function(user, item_id)
    user:ResetDimensionInout(3)
    user:ResetDimensionInout(4)
    user:ResetDimensionInout(5)
...
-- 以下是异界重置券代码 上面是E2重置，下面是E3重置 
item_handler[490701077] = function(user, item_id)
    user:ResetDimensionInout(0)
    user:ResetDimensionInout(1)
    user:ResetDimensionInout(2)
	user:SendNotiPacketMessage("恭喜： 异界E2重置 成功！")
end
item_handler[490701078] = function(user, item_id)
    user:ResetDimensionInout(3)
    user:ResetDimensionInout(4)
    user:ResetDimensionInout(5)
	user:SendNotiPacketMessage("恭喜： 异界E3重置 成功！")
...
-- 成就任务完成
item_handler[490703500] = function(user, item_id)
    local quest = dpx.quest
    local lst = quest.all(user.cptr)
    local chr_level = user:GetCharacLevel()
...
item_handler[690000832] = function(user, item_id)
    local mask = game.InheritMask.FLAG_UPGRADE | game.InheritMask.FLAG_AMPLIFY | game.InheritMask.FLAG_ENCHANT | game.InheritMask.FLAG_SEPARATE
    mask = mask | game.InheritMask.FLAG_MOVE_UPGRADE | game.InheritMask.FLAG_MOVE_AMPLIFY | game.InheritMask.FLAG_MOVE_ENCHANT | game.InheritMask.FLAG_MOVE_SEPARATE
    ...
    elseif item1.type ~= item2.type then
        user:SendNotiPacketMessage("注意：相同的装备类型才可以继承！", 1)
    elseif item1.rarity ~= item2.rarity  then
        user:SendNotiPacketMessage("注意：品级相同才可以继承！", 1)
    elseif math.abs(item2.usable_level - item1.usable_level) >= 10 then
        user:SendNotiPacketMessage("注意：等级差必须小于等于10级的装备才可以被继承！", 1)
    elseif item1.usable_level < 50 or item2.usable_level < 50   then
        user:SendNotiPacketMessage("注意：低于50级的装备不可以继承！", 1)
    elseif item1.amplify.type == 0 and item2.amplify.type ~= 0  then
        user:SendNotiPacketMessage("注意：强化装备不可以继承给增幅装备！", 1)
    elseif dpx.item.inherit(user.cptr, 9, 10, mask) then
        return user:SendNotiPacketMessage("恭喜：已经成功继承！", 1)
    else
        user:SendNotiPacketMessage("注意：很遗憾继承失败！", 1)
    end
    dpx.item.add(user.cptr, item_id)
end

-- 以下是宠物删除券代码 删除宠物前2栏 
item_handler[490702596] = function(user, item_id)
    local q = 0
    for i = 0, 13, 1 do
        local info = dpx.item.info(user.cptr, 7, i)
        if info then
...
-- 以下是时装删除券代码 删除时装前2栏
item_handler[490702597] = function(user, item_id)
    local q = 0
	for i = 0, 13, 1 do
		local info = dpx.item.info(user.cptr, 1, i)
		if info then
...
-- 副职业一键分解券 分解装备前3栏 需要在开启个人分解机状态下使用
...
---@param user CUser
item_handler[690000836] = function(user, item_id)
    user:AddCharacExpPercent(0.1) -- add 10% lv exp
end
...
---@param user CUser
item_handler[10000215] = function(user, item_id)
    user:AddCharacExpPercent(1.0) -- add 100% lv exp
end
...
---@param user CUser
item_handler[690000431] = function(user, item_id)
    user:AddCharacExpPercent(1.0) -- add 100% lv exp
end
...
---@param user CUser
item_handler[10000213] = function(user, item_id)
    user:AddCharacExpPercent(1.0) -- add 100% lv exp
end
...
---@param user CUser
item_handler[10000216] = function(user, item_id)
    user:AddCharacExpPercent(0.1) -- add 10% lv exp
end
...
---@param user CUser
item_handler[490702579] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 20899, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[2682006] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 10027, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490702576] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 136, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490702577] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 142, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490702578] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 138, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490702586] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 4620, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490702585] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 4619, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490702584] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 4618, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490702583] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 4617, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490702582] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 4616, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490702581] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 4615, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490702580] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 4534, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
end
...
---@param user CUser
item_handler[490703499] = function(user, item_id)
    local ok = dpx.quest.accept(user.cptr, 94, true)
    if not ok then
        dpx.item.add(user.cptr, item_id)
    end
...
--以下为副本难度全开，不用就删掉这一段--
item_handler[2660372] = function(user, item_id)
...
--dpx.hook(game.HookType.GmInput, on_input)--对于游戏内输入指令反馈和结果[暂注释, 由主脚本df_game_r.lua的GmInput处理GM指令]
