...
---------------------------------- GM指令处理(调用JavaScript实现, 参考yt分支) -------------------------- !
-- 玩家在游戏内输入 //item 物品代码 物品数量 / //renwu 完成任务, 经lua转发给frida处理
local on_input = function(fnext, _user, input)
    ...
    if string.sub(input, 1, 2) == "//" then
        cmd = string.sub(input, 3)
    end
    local data = string.format("{\"user_ptr\":\"%s\",\"cmd\":\"%s\"}", tostring(user.cptr), cmd)
    local result = frida.call(1, 0, data)
    if result == 0 then
        return 0
    end
...
dpx.hook(game.HookType.GmInput, on_input)-- 游戏内输入指令处理(GM指令经lua转发给frida处理) !
