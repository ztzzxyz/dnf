...
const EVENT_VILLAGEATTACK_START_HOUR = 21; //每日北京时间20点开启活动
...
function api_CItem_GetItemName(item_id) {
    //api_GameWorld_SendNotiPacketMessage('玩家[', 14);
...
function countQuality4EquipsInCurrentInstance(user, charac_no) {
    ...
    console.log(identification + api_CUserCharacInfo_getCurCharacName(user) + ']获得史诗装备:' + userCounters[charac_no] + '件');
...
function getname2(item_id) {
    var citem1 = CDataManager_find_item(G_CDataManager(), item_id);
    //if (!citem1.isNull()) {
    return CItem_GetItemName(citem1).readUtf8String(-1);
    //}
    //return item_id.toString();
}
...
//start
                    ...
                    var EquRarity0 = CItem_GetRarity(itemData);

                    if (reason == 4)//副本内拾取
                    {

                        //-----------------------------------下方为捡史诗就会公告，如果不需要整段删除-----------------------------------
                        if (EquRarity0 == 3 || EquRarity0 == 4)//要求品级为粉
                        {
                            ...
                            //var citem1 = CDataManager_find_item(G_CDataManager(), item_id);
                            //var name111 = CItem_GetItemName(citem1).readUtf8String(-1);
                            //var name111 = getname2(item_id);

                            api_GameWorld_SendNotiPacketMessage('玩家[' + api_CUserCharacInfo_getCurCharacName(user) + ']于' + segmentation3 + '在' + dgnname[charac_no] + '获得[' + getname2(item_id) + ']', 14);
                            //api_GameWorld_SendNotiPacketMessage('玩家[', 14);
                        ...
                        if ((item_id >= 33714) && (item_id <= 33714))//33714是魔杖-极光魅影的代码，可以写区间代码，例如if((item_id >= 33714) && (item_id <= 33715))，那就是爆出33715双龙魔杖也会公告。
                        {
                            //-------------下方为奖励无色，需要则替换代码和数量，不需要则删除-------------
                            yb_reward = 3037 //奖励无色x5
                            number = 5 //5个
...
//end
                    ...
                    if (mob_id == 1) //如果怪物代码等于1
                    {
                        ...
                        {
                            yb_reward = 3038 //黑色大晶体
                            number = 1 //1个
                            //api_GameWorld_SendNotiPacketMessage('哥布林:只会虐菜,你死得了!', 1)
                            //sendRewardsByWindow(user, [yb_reward, number]);//发放道具奖励黑色大晶体1个
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                else if (game_event == 'DungeonClearInfo') //副本信息清除
                {
                    ...
                    //api_CUser_SendNotiPacketMessage(user, '命运已被改变, 当前幸运点数: ' + savedFirstSecondsValue, 0);
                    var identification = '玩家[';
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                    //if (userCounters[charac_no] == 2) //如果个人在副本中获得史诗装备2件
                    //	{
                    //	api_CUser_SendNotiPacketMessage(user, '命运已被改变, 当前幸运点数: ' + savedFirstSecondsValue, 0);
                    //	var randomCash = Math.floor(Math.random() * (20000 - 500 + 1)) + 500; // 随机生成500到20000之间的点卷数量
                    //	var randomItemNumber = Math.floor(Math.random() * (400 - 20 + 1)) + 20; // 随机生成20到400之间的物品数量
                    //	yb_reward = 3037; //奖励无色
                    //	api_GameWorld_SendNotiPacketMessage('--------------鸿运当头--------------\n恭喜玩家[' + api_CUserCharacInfo_getCurCharacName(user) + ']在' + Dungeon_name + '爆出2黄\n☆奖励☆[点券+' + randomCash + ']\n☆奖励☆[' + getname2(yb_reward)+ 'x' + randomItemNumber + ']', 14);
                    //	sendRewardsByWindow(user, [yb_reward, randomItemNumber]);//发放道具奖励
                    //	api_CUser_AddItem(user, yb_reward, randomItemNumber);
                    //	api_recharge_cash_cera(user, randomCash);//发放点券奖励
                    //	}
                    // else if (userCounters[charac_no] == 3) //如果个人在副本中获得史诗装备1件
                    //		{
                    //		var randomCash = Math.floor(Math.random() * (20000 - 500 + 1)) + 500; // 随机生成500到20000之间的点卷数量
                    //		var randomItemNumber = Math.floor(Math.random() * (400 - 20 + 1)) + 20; // 随机生成20到400之间的物品数量
                    //		yb_reward = 3037 //奖励无色
                    //		api_GameWorld_SendNotiPacketMessage('--------------鸿运当头--------------\n恭喜玩家[' + api_CUserCharacInfo_getCurCharacName(user) + ']在' + Dungeon_name + '爆出3黄\n☆奖励☆[点券+' + randomCash + ']\n☆奖励☆[' + getname2(yb_reward)+ 'x' + randomItemNumber + ']', 14);
                    //		sendRewardsByWindow(user, [yb_reward, randomItemNumber]);//发放道具奖励
                    //		api_CUser_AddItem(user, yb_reward, randomItemNumber);
                    //		api_recharge_cash_cera(user, randomCash);//发放点券奖励
                    //		}
                    // else if (userCounters[charac_no] == 4) //如果个人在副本中获得史诗装备1件
                    //		{
                    //		var randomCash = Math.floor(Math.random() * (20000 - 500 + 1)) + 500; // 随机生成500到20000之间的点卷数量
                    //		var randomItemNumber = Math.floor(Math.random() * (400 - 20 + 1)) + 20; // 随机生成20到400之间的物品数量
                    //		yb_reward = 3037 //奖励无色
                    //		api_GameWorld_SendNotiPacketMessage('--------------鸿运当头--------------\n恭喜玩家[' + api_CUserCharacInfo_getCurCharacName(user) + ']在' + Dungeon_name + '爆出4黄\n☆奖励☆[点券+' + randomCash + ']\n☆奖励☆[' + getname2(yb_reward)+ 'x' + randomItemNumber + ']', 14);
                    //		sendRewardsByWindow(user, [yb_reward, randomItemNumber]);//发放道具奖励
                    //		api_CUser_AddItem(user, yb_reward, randomItemNumber);
                    //		api_recharge_cash_cera(user, randomCash);//发放点券奖励
                    //		}
                    // else if (userCounters[charac_no] == 5) //如果个人在副本中获得史诗装备1件
                    //		{
                    //		var randomCash = Math.floor(Math.random() * (20000 - 500 + 1)) + 500; // 随机生成500到20000之间的点卷数量
                    //		var randomItemNumber = Math.floor(Math.random() * (400 - 20 + 1)) + 20; // 随机生成20到400之间的物品数量
                    //		yb_reward = 3037 //奖励无色
                    //		api_GameWorld_SendNotiPacketMessage('--------------鸿运当头--------------\n恭喜玩家[' + api_CUserCharacInfo_getCurCharacName(user) + ']在' + Dungeon_name + '爆出5黄\n☆奖励☆[点券+' + randomCash + ']\n☆奖励☆[' + getname2(yb_reward)+ 'x' + randomItemNumber + ']', 14);
                    //		sendRewardsByWindow(user, [yb_reward, randomItemNumber]);//发放道具奖励
                    //		api_CUser_AddItem(user, yb_reward, randomItemNumber);
                    //		api_recharge_cash_cera(user, randomCash);//发放点券奖励
                    //		}
                    // else if (userCounters[charac_no] == 6) //如果个人在副本中获得史诗装备1件
                    //		{
                    //		var randomCash = Math.floor(Math.random() * (20000 - 500 + 1)) + 500; // 随机生成500到20000之间的点卷数量
                    //		var randomItemNumber = Math.floor(Math.random() * (400 - 20 + 1)) + 20; // 随机生成20到400之间的物品数量
                    ////		yb_reward = 3037 //奖励无色
                    //		api_GameWorld_SendNotiPacketMessage('--------------鸿运当头--------------\n恭喜玩家[' + api_CUserCharacInfo_getCurCharacName(user) + ']在' + Dungeon_name + '爆出6黄\n☆奖励☆[点券+' + randomCash + ']\n☆奖励☆[' + getname2(yb_reward)+ 'x' + randomItemNumber + ']', 14);
                    //		sendRewardsByWindow(user, [yb_reward, randomItemNumber]);//发放道具奖励
                    //		api_CUser_AddItem(user, yb_reward, randomItemNumber);
                    //		api_recharge_cash_cera(user, randomCash);//发放点券奖励
                    //		}
                    // else if (userCounters[charac_no] == 7) //如果个人在副本中获得史诗装备1件
                    //		{
                    //		var randomCash = Math.floor(Math.random() * (20000 - 500 + 1)) + 500; // 随机生成500到20000之间的点卷数量
                    //		var randomItemNumber = Math.floor(Math.random() * (400 - 20 + 1)) + 20; // 随机生成20到400之间的物品数量
                    //		yb_reward = 3037 //奖励无色
                    //		api_GameWorld_SendNotiPacketMessage('--------------鸿运当头--------------\n恭喜玩家[' + api_CUserCharacInfo_getCurCharacName(user) + ']在' + Dungeon_name + '爆出7黄\n☆奖励☆[点券+' + randomCash + ']\n☆奖励☆[' + getname2(yb_reward)+ 'x' + randomItemNumber + ']', 14);
                    //		sendRewardsByWindow(user, [yb_reward, randomItemNumber]);//发放道具奖励
                    //		api_CUser_AddItem(user, yb_reward, randomItemNumber);
                    //		api_recharge_cash_cera(user, randomCash);//发放点券奖励
                    //		}
...
//-------------------------------------------------------------------------------------------------------------------------------------------------------------------------
                else if (game_event == 'MailS') //邮件发送
                {
                    ...
                    console.log(identification + api_CUserCharacInfo_getCurCharacName(user) + ']发送邮件[' + getname2(item_id) + '] ' + Number_of_emails + ' 个,到角色[' + js_name + ']');
                }
...
function UserUseItemEvent(user, item_id) {
    ...
    } else if (2023889675 == item_id) {//继承券
        equInherit(user, item_id);
    } else if (2023889808 == item_id) {//副本难度
        unlock_all_dungeon_difficulty(user);
        //api_CUser_SendNotiPacketMessage(user,"已解锁所有副本难度，请当前角色小退刷新",1);
        send_windows_pack_233(user, '已解锁所有副本难度，请当前角色小退刷新。');
    } else if (2023889044 == item_id) {//锻造券
        duanzaoquan(user, 7, item_id);
    } else if (2023889045 == item_id) {//锻造券
        duanzaoquan(user, 8, item_id);
    } else if (2023889046 == item_id) {//锻造券
        duanzaoquan(user, 9, item_id);
    } else if (2023889047 == item_id) {//锻造券
        duanzaoquan(user, 10, item_id);
    } else if (2023889048 == item_id) {//锻造券
        duanzaoquan(user, 11, item_id);
    } else if (2023889049 == item_id) {//锻造券
        duanzaoquan(user, 12, item_id);
    } else if (2023889050 == item_id) {//锻造券
        duanzaoquan(user, 13, item_id);
    } else if (2023889051 == item_id) {//锻造券
        duanzaoquan(user, 14, item_id);
    } else if (2023889052 == item_id) {//锻造券
        duanzaoquan(user, 15, item_id);
    }
}

//继承
function equInherit(user, item_inherit) {
    var inven = CUserCharacInfo_getCurCharacInvenW(user);
    ...
    if (itemId) {
        //读取装备强化等级
        var upgrade_level = equ.add(6).readU8();
        upgrade_level = upgrade_level % 32;
            ...
            if (Inven_Item_getKey(equIn)) {
                ...
                if (inEqu_type == equ_type) {
                    if (inEqu_type == 10) {
                        ...
                        if (useJob != inUseJob) {
                            api_CUser_SendNotiPacketMessage(user, "继承失败：武器装备需要当前职业且同类型，穿戴装备不满足要求", 1);
                            api_CUser_AddItem(user, item_inherit, 1);
                            return;
                        }
                    ...
                    // 类型一直 才能继承
                    if (inEquRarity < 1) {
                        // 继承失败：装备品级必须要求蓝色以上，穿戴装备不满足要求
                        //api_CUser_SendNotiPacketMessage(user,"继承失败：装备品级必须要求粉色以上，穿戴装备不满足要求",1);
                        send_windows_pack_233(user, '继承失败：装备品级必须要求粉色以上，穿戴装备不满足要求。');
                        api_CUser_AddItem(user, item_inherit, 1);
                        return;
                    }
                    if (inNeedLevel < 10) {
                        // 装备等级要大于10级以上，穿戴装备不满足要求
                        //api_CUser_SendNotiPacketMessage(user,"继承失败：装备等级要大于等于15级以上，穿戴装备不满足要求",1);
                        send_windows_pack_233(user, '继承失败：装备等级要大于等于15级以上，穿戴装备不满足要求。');
                        api_CUser_AddItem(user, item_inherit, 1);
                        return;
                    }
                    // 强化
                    var inUpgrade_level = (equIn.add(6).readU8()) % 32;
                    // 增幅
                    ...
                    if (baozhu == 0) {
                        api_CUser_AddItem(user, item_inherit, 1);
                        //api_CUser_SendNotiPacketMessage(user,"继承失败：父装备无附魔属性！",1);
                        send_windows_pack_233(user, '继承失败，父装备无附魔属性');
                        return;
                    } else if (inUpgrade_level > 22) {
                        api_CUser_AddItem(user, item_inherit, 1);
                        api_CUser_SendNotiPacketMessage(user, "继承失败：穿戴装备强化/增幅属性大于22，不可以被继承！", 1);
                        return;
                    }
                    //提升强化/增幅等级
                    ...
                    console.log("success！！！")
                    if (inEqu_type == 10) {
                        //api_CUser_SendNotiPacketMessage(user,"继承成功！继承武器需要重新登录角色",1);
                        send_windows_pack_233(user, '继承成功！继承武器需要重新登录角色');
                    } else {
                        //api_CUser_SendNotiPacketMessage(user,"继承成功！",1);
                        send_windows_pack_233(user, '         继承成功！');
                    }
                    break;
                }
            }
        ...
        api_CUser_SendNotiPacketMessage(user, "继承失败，第一格无装备！", 1);
        api_CUser_AddItem(user, item_inherit, 1);
    }
}

//锻造
function duanzaoquan(user, num, id_duanzao) {
    var inven = CUserCharacInfo_getCurCharacInvenW(user);
    ...
    if (itemId) {
        ...
        if (equRarity < 2) {
            ...
            ('<怪物攻城活动>', '恭喜勇士!', reward_gold, reward_item_list);

            var charac_no1 = CUserCharacInfo_getCurCharacNo(user);
            var reward_item_list =
                [
                    [7745, 5 * (1 + villageAttackEventInfo.difficult)], //士气冲天
                    [2600028, 5 * (1 + villageAttackEventInfo.difficult)], //天堂痊愈
            ...
            api_WongWork_CMailBoxHelper_ReqDBSendNewSystemMultiMail(charac_no, '<装备锻造券回退>', '请继续锻造吧!', 0, item_list);


            api_CUser_AddItem(user, id_duanzao, 1);
            api_CUser_SendNotiPacketMessage(user, "锻造失败：装备品级必须要求紫色以上", 1);
            return;
        } else if (equ_type != 10) {
            api_CUser_AddItem(user, id_duanzao, 1);
            api_CUser_SendNotiPacketMessage(user, "锻造失败：装备必须是武器！", 1);
            return;
        } else if (duanzao >= num) {
            api_CUser_AddItem(user, id_duanzao, 1);
            api_CUser_SendNotiPacketMessage(user, '锻造失败：当前装备锻造等级大于等于' + num, 1);
            return;
        } else {
            api_CUser_SendNotiPacketMessage(user, "锻造成功，重新登录角色生效！！！", 1);
            equ.add(51).writeU8(num);
            CUser_SendUpdateItemList(user, 1, 0, 9);
        }
    } else {
        api_CUser_AddItem(user, id_duanzao, 1);
        api_CUser_SendNotiPacketMessage(user, "锻造失败：武器栏无装备！", 1);
    }
}
...
function frida_handler(ls, arg1, arg2, _arg3) {
    ...
    if (arg1 == 1) {
        try {
            ...
            if (!(api_MySQL_exec(mysql_frida, "select VIP from d_taiwan.accounts where UID = '" + M_id + "' and VIP = 1;") && MySQL_get_n_rows(mysql_frida) == 1)) {
                MySQL_fetch(mysql_frida);
                api_CUser_SendNotiPacketMessage(user, '无GM命令权限', 2);
                return;
            }
            var cmd = data.cmd;
